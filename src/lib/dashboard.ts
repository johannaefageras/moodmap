...
function describeEntry(r: EntryRow): string {
  ...
  if (r.concentration !== null) parts.push(`Koncentration ${fmtNum(r.concentration)}/10`)
  ...
  else if (r.sleep_quality !== null) parts.push(`Sömnkvalitet ${fmtNum(r.sleep_quality)}/10`)
  ...
  if (r.social_relation) parts.push(`Socialt · ${SOCIAL_LABEL[r.social_relation] ?? r.social_relation}`)
  return parts.join(' · ')
}
...
export const getDashboardSummary = createServerFn({ method: 'GET' }).handler(
  async (): Promise<DashboardSummary> => {
    ...
    const seriesSum = (key: keyof EntryRow): number[] =>
      dayKeys.map((d) => sum((byDay.get(d) ?? []).map((r) => r[key] as number | null | undefined)))
    const seriesSocial = (): number[] =>
      dayKeys.map((d) => (byDay.get(d) ?? []).filter((r) => r.social_relation !== null).length)
    ...
    return {
      ...
      avg7d: {
        ...
        socialPerDay: last7.filter((r) => r.social_relation !== null).length / 7,
...
export const getChartSeries = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }): Promise<ChartSeries> => {
    ...
    const { data: rows, error } = await supabase
      .from('entries')
      .select('logged_for, mood, energy, sleep_hours, anxiety, stress, concentration')
      .eq('user_id', user.id)
